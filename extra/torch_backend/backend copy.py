...
#   st = UOp(Ops.AS_STRIDED, dtype=tensor.dtype, src=(tensor.uop,), arg=(tuple(size), tuple(stride), storage_offset))
...
def _as_strided(tensor: Tensor, size, stride, storage_offset=None):
  ...
  u = UOp(Ops.AS_STRIDED, dtype=base.dtype, src=(base.uop,), arg=(size, stride, off))
  ret = Tensor.from_uop(u)
  if prod(size) == 1: return ret
